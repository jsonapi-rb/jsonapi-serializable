...
describe JSONAPI::Serializable::AbstractResource, '#as_jsonapi' do
  before(:all) do
    @users = [
  ...
  it 'handles bare minimum' do
    user_klass = Class.new(JSONAPI::Serializable::AbstractResource) do
      type 'users'
      id { @user.id.to_s }
    end
  ...
  it 'does not include data when relationship is not included' do
    post_klass = Class.new(JSONAPI::Serializable::AbstractResource) do
      type 'posts'
    end
    user_klass = Class.new(JSONAPI::Serializable::AbstractResource) do
      type 'users'
